    ...
    <div className="glass rounded-2xl p-6 space-y-5">
      ...
      {renderSelect('district', 'Kecamatan *', formData.districtId, loadingDistricts, !formData.cityId, 'Pilih Kecamatan',
        ...
        (val) => {
          ...
          update({ districtId: val, districtName: dist?.name || '' });
        }
      )}

      <div>
        <Label htmlFor="kelurahan">Desa / Kelurahan *</Label>
        <Input id="kelurahan" required value={formData.kelurahan} onChange={(e) => update({ kelurahan: e.target.value })} className="bg-muted/50 border-border mt-1.5" placeholder="Nama desa/kelurahan" />
      </div>
